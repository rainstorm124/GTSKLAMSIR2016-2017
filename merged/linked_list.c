...
/*
operation:		initialize and return a pointer to node
postconditions:	data copied to data member with strdup to avoid issues, 
				both parent and child node initialized to NULL
*/
node* create_node(void *data){
	node *n = calloc(1, sizeof(struct node));	// Allocate memory
	n->data = data;					// Set node's data
	n->parent = n->child = NULL;	// Initialize both parent and child to NULL
	return n;
}

/*
operation:		Return the node in a linked list with given data, if found.
postconditions:	Intended operation, otherwise returning NULL if not found.
*/
node* get_node(linked_list *list, void *data){
	node *n = list->head;					// start search at head
	while(n){									// loop while n is not NULL
		if(list->compare(n->data, data) == 0){	// compare current data with given data
			return n;							// return current node: match found
		}
...
void add_node(linked_list *list, node *n){
	if(list->size == 0){
		list->head = n;					// list is empty, node becomes head by default and subsequently:
		list->head->parent = n;			// head's child is itself
		list->head->child = NULL;		// head's parent is itself
	}else{									
		n->parent = list->head->parent;	// set node's parent to tail
		n->child = NULL;				// set node's child to head
		list->head->parent->child = n;	// set tail's child to node
		list->head->parent = n;			// set head's parent to node
	}
	list->size++;						// increment size of list by 1
}

/*
operation:		Print the nodes and their relationships in a linked list
*/
void print_list(linked_list *list){
	if(!list->head){
		printf("<List is empty>\n");		// list is empty, nothing to print
	}
	node *n = list->head;				// start iteration at head
	while(n){								// loop while n is not NULL
...
/*
operation: Free every node of the linked list, points head to NULL and sets size to 0
*/
void empty_list(linked_list *list){
	if(!list->head) return;
	node *parent, *temp = list->head;
...
node* get_node_at(linked_list *list, int pos){
...
/*
operation:		Insert a node in place of another and re-link accordingly.
...
*/
bool insert_node(linked_list *list, node *dst, node *src){
	if(!contains_node(list, dst)){
...
/*
operation:		Determine if given node is contained in a list.
postconditions:	Returns true if a match is found, false if not.
*/
bool contains_node(linked_list *list, node *src){
	node *n = list->head;	// start search at head
	while(n){					// loop while n is not NULL
		if(n == src){			// compare each node with source
...
/*
operation:		Delete a node from a list and re-link accordingly.
...
*/
bool delete_node(linked_list *list, node *src){
	if(!contains_node(list, src)){
		return false;						// return false: not contained
	}else{
		node *tail = list->head;
