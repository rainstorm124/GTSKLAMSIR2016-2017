...
int main(void){
  print_header();
	...
	char *round_HTML_setup = malloc(sizeof(char)*128);
	char *prompt_HTML_setup = malloc(sizeof(char)*128);
	char *user_HTML_setup = malloc(sizeof(char)*128);
	...
	char **nav_split_by_line = split(navigatorfile, '\n');
	char *choicesfile = read_text("update_file.txt");
	char **update_split_by_line = split(choicesfile, '\n');
	char ***nav_lines_split = calloc(sizeof(char**), 1024);
	for (int i = 0; nav_split_by_line[i]; i++){
		char **split_nav_line = split(nav_split_by_line, '=');
    nav_lines_split[i] = split_nav_line;
...
//  strcpy(nav_lines_split[i][1], split_nav_line[1]);
	...
	for (int i = 0; update_split_by_line[i]; i++){
		char **split_update_line = split(update_split_by_line[i], ';');
		int j = 0;
		...
		if(strcmp(split_update_line[0], nav_lines_split[j][0])==0)
			printf("<tr><td>%s</td><td>%s</td></tr>", nav_lines_split[j][0], "true");
		else
			printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", nav_lines_split[j][0], "false");
	}
	printf("</table></center></div>");
	printf("</body> </html>");
